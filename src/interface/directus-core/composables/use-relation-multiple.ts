// Based on https://github.com/directus/directus/blob/main/app/src/composables/use-relation-multiple.ts
...
import type { RelationM2M } from "./types/relations";
// import { RelationO2M } from "@/composables/use-relation-o2m";
import { RelationO2M } from "./types/relations";
...
export function useRelationMultiple(
    ...
    const { cleanItem, getPage, isLocalItem, getItemEdits, isEmpty } =
        useUtil();
    ...
    const _value = computed<ChangesItem>({
        ...
        set(newValue) {
            if (
                newValue.create.length === 0 &&
                newValue.update.length === 0 &&
                newValue.delete.length === 0
            ) {
    ...
    watch(value, (newValue, oldValue) => {
        if (
            ...
            oldValue &&
            (("create" in oldValue &&
                Array.isArray(oldValue.create) &&
                oldValue.create.length > 0) ||
                ("update" in oldValue &&
                    Array.isArray(oldValue.update) &&
                    oldValue.update.length > 0) ||
                ("delete" in oldValue &&
                    Array.isArray(oldValue.delete) &&
                    oldValue.delete.length > 0))
        ) {
            updateFetchedItems();
        }
    });

    watch([previewQuery, itemId, relation], updateFetchedItems, {
        immediate: true,
    });

    const { fetchedSelectItems, selected, isItemSelected, selectedOnPage } =
        useSelected();
    ...
    const createdItems = computed(() => {
        ...
        return items.filter(
            (item) => item[info.reverseJunctionField.field] === undefined
        );
    });
    ...
    const displayItems = computed(() => {
        ...
        const items: DisplayItem[] = fetchedItems.value.map(
            (item: Record<string, any>) => {
                const editsIndex = _value.value.update.findIndex(
                    (edit) =>
                        typeof edit === "object" &&
                        edit[targetPKField] === item[targetPKField]
                );

                const deleteIndex = _value.value.delete.findIndex(
                    (id) => id === item[targetPKField]
                );
                ...
                if (editsIndex !== -1) {
                    ...
                    if (
                        relation.value?.type === "m2m" ||
                        relation.value?.type === "m2a"
                    ) {
                ...
                switch (relation.value?.type) {
                    ...
                    case "m2m":
                        return (
                            ...
                            item[relation.value.junctionField.field][
                                relation.value.relatedPrimaryKeyField.field
                            ]
                        );
                    ...
                    case "m2a": {
                        ...
                        const editCollection =
                            edit[relation.value.collectionField.field];
                        const itemPkField =
        ...
        const newItems = getPage(
            existingItemCount.value + selected.value.length,
            createdItems.value
        );

        items.push(...fullSelectedOnPage, ...newItems);

        const sortField = (previewQuery.value.sort ??
            toArray(relation.value.sortField))[0];

        if (
            (previewQuery.value.limit > 0 &&
                totalItemCount.value > previewQuery.value.limit) ||
            !sortField
        )
            return items;
            ...
            const selected = items!.map((item) => {
                switch (info.type) {
                    ...
                    case "m2a": {
                        if (!collection)
                            throw new Error(
                                "You need to provide a collection on an m2a"
                            );
    ...
    async function updateFetchedItems() {
        ...
        switch (relation.value.type) {
            ...
            case "m2m":
                ...
                fields.add(relation.value.junctionPrimaryKeyField.field);
                fields.add(
                    `${relation.value.junctionField.field}.${relation.value.relatedPrimaryKeyField.field}`
                );
                break;
            case "o2m":
        ...
        try {
            ...
            if (itemId.value !== "+") {
                const filter: Filter = {
                    _and: [{ [reverseJunctionField]: itemId.value } as Filter],
                };
    ...
    async function updateItemCount() {
        ...
        existingItemCount.value = Number(
            response.data.data[0].count[targetPKField]
        );
    }
    ...
    function useSelected() {
        ...
        const selectedOnPage = computed(() =>
            getPage(existingItemCount.value, selected.value)
        );
        ...
        watch(
            ...
            (newVal, oldVal) => {
                if (
                    ...
                    !isEqual(
                        newVal.map(getRelatedIDs),
                        (oldVal ?? []).map(getRelatedIDs)
                    )
                ) {
                    loadSelectedDisplay();
                }
        ...
        async function loadSelectedDisplayO2M(relation: RelationO2M) {
            ...
            fetchedSelectItems.value = await fetchAll(
                getEndpoint(targetCollection),
                {
                    params: {
                        fields: Array.from(fields),
                        filter: {
        ...
        async function loadSelectedDisplayM2M(relation: RelationM2M) {
            ...
            const response = await fetchAll<Record<string, any>[]>(
                getEndpoint(relation.relatedCollection.collection),
                {
                    params: {
                        fields: Array.from(fields),
                        filter: {
            ...
            fetchedSelectItems.value = response.map(
                (item: Record<string, any>) => ({
            ...
            fetchedSelectItems.value = responses.reduce((acc, item, index) => {
                acc.push(
                    ...item.map((item: Record<string, any>) => ({
                        [relation.collectionField.field]:
                            Object.keys(selectGrouped)[index],
                        [relation.junctionField.field]: item,
                    }))
                );
    ...
    function useUtil() {
        ...
        function isEmpty(item: DisplayItem): boolean {
            if (item.$type !== "updated" && item.$edits === undefined)
                return false;

            const topLevelKeys = Object.keys(item).filter(
                (key) => !key.startsWith("$")
            );
            ...
            if (relation.value?.type === "o2m") {
                ...
                const deepLevelKeys = Object.keys(
                    item[relation.value.junctionField.field]
                );
                ...
                return (
                    ...
                    topLevelKeys.includes(
                        relation.value.collectionField.field
                    ) &&
                    deepLevelKeys.length === 1
                );
            }
        ...
        function isLocalItem(item: DisplayItem) {
            return (
                item.$type !== undefined &&
                (item.$type !== "updated" || isItemSelected(item))
            );
        }
        ...
        function getPage<T>(offset: number, items: T[]) {
            ...
            const start = clamp(
                (previewQuery.value.page - 1) * previewQuery.value.limit -
                    offset,
                0,
                items.length
            );
            const end = clamp(
                previewQuery.value.page * previewQuery.value.limit - offset,
                0,
                items.length
            );
