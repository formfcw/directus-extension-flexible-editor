...
export function useRelationM2A(collection: Ref<string>, field: Ref<string>) {
    // [START] different from original
    const { useCollectionsStore, useFieldsStore, useRelationsStore } =
        useStores();
    // [END] different from original
    ...
    const relationInfo = computed<RelationM2A | undefined>(() => {
        ...
        return {
            ...
            collectionField: fieldsStore.getField(
                junction.collection,
                relation.meta?.one_collection_field as string
            ),
            junctionCollection: collectionsStore.getCollection(
                junction.collection
            ),
            junctionPrimaryKeyField:
                fieldsStore.getPrimaryKeyFieldForCollection(
                    junction.collection
                ),
